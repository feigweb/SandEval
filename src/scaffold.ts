...
export async function scaffoldCustomProvider(cwd: string, targetPath: string, overwrite = false): Promise<string> {
  const resolved = path.isAbsolute(targetPath) ? targetPath : path.resolve(cwd, targetPath);
  if (existsSync(resolved) && !overwrite) {
    throw new Error(`File already exists: ${resolved}. Pass --force to overwrite.`);
  }
  await mkdir(path.dirname(resolved), { recursive: true });
  await writeFile(resolved, providerTemplate(), "utf8");
  return resolved;
}
